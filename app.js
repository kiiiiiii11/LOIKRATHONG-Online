...
function addKrathong(wish, xPosition) {
    ...
    document.body.appendChild(krathong);
}

// ดึงข้อมูลกระทงจาก Firebase
onChildAdded(krathongsRef, (snapshot) => {
    const data = snapshot.val();
    addKrathong(data.wish, data.x);
});
...
function createLantern() {
    ...
    lantern.style.animationDuration = `${Math.random() * 10 + 5}s`;
    document.body.appendChild(lantern);
}

// สร้างโคมลอยทุก 3 วินาที
